...
// [호출 흐름]
// View : 사용자의 입력을 받아서 처리 -> Service(규칙 검사) -> DAO(SQL 실행) -> DB(요청, 응답)
public class LibraryService {
    // 인터페이스 설계 - 여기서는 X

    // Service 하나가 DAO 3개를 소유할 수 있다.
    // 포함 관계
    ...
    // [BookDAO]
    // 1. 도서 추가 기능
    // BookDAO는 쿼리에 집중하는 구조이다.
    // Service에서는 제약사항 확인을 해서 던져야 한다.
    // (1) 제목과 저자가 비어 있는지 확인(둘 중 하나라도 없으면 중단)
    // (2) 통과하면 DAO에게 INSERT 처리를 위임한다.
    public void addBook(Book book) throws SQLException {
        if (book.getTitle() == null || book.getTitle().trim().isEmpty() ||
                book.getAuthor() == null || book.getAuthor().trim().isEmpty()) {
            throw new SQLException("도서 제목과 저자는 필수 항목입니다.");
        }
        bookDAO.addBook(book);
    }

    // 2. 전체 도서 조회
    // 검사할 규칙이 없으므로 DAO에 결과를 그대로 넘긴다.
    ...
    // 4. 학생 등록
    // (1) 이름과 학번이 비어있는지 검사
    // (2) 통과하면 DAO에 INSERT 처리를 위임
    // UNIQUE 걸려있는 student_id는 DB에서 확인해야하므로 여기서는 먼저 중복 검사를 하지 않는다.
    ...
    // 8. 도서 대출
    // (1) 도서 ID와 학생 ID가 1 이상인지 검사(auto-increment는 1부터 시작)
    // (2) 통과하면 DAO의 트랜잭션 메서드에 위임
    // (3) 사실 View 단에서 로그인 여부를 확인하고 수행할 수 있도록 처리가 된다.
    ...
    // 관리자 로그인(ID와 비밀번호를 확인)
    // [처리순서]
    // (1) ID와 비밀번호가 비어있는지 검사
    // (2) DAO 에게 해당 ID의 관리자 정보를 찾는다. (없으면 null)
    // (3) 사용자가 입력한 비밀번호와 DB에 저장된 비밀번호를 비교한다.
    // (4) 일치하면 비밀번호를 지운 Admin 객체를 반환, 아니면 null을 View클래스에 돌려준다.
    public Admin authenticateAdmin(String adminId, String password) throws SQLException {
        ...
        // (4) 인증이 끝난 객체에 비밀번호를 남겨둘 이유가 없으므로 지우고 돌려준다.
